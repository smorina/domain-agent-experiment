...
Develop a comprehensive prompt (context file) that instructs a powerful AI agent (GPT-5-Pro) to gather and compile knowledge about **domain modeling** and **domain engineering**. The goal is to equip a coding assistant with expert understanding and methodology for creating domain models, especially for critical, high-assurance systems. This prompt should guide GPT-5-Pro to find relevant sources (academic papers, books, etc.), extract actionable insights, and produce a **technical, structured Markdown document** that defines the domain-modeling agent’s knowledge and behavior.

## **Research Focus and Requirements**

The prompt for GPT-5-Pro should direct it to cover the following key areas:
...
* **Best Practices and Principles for Domain Modeling:** Direct GPT-5-Pro to compile **actionable best practices** that the agent should follow while assisting in domain modeling:
  ...
  * *Adaptability:* Importantly, the agent should **choose the modeling approach appropriate to the task at hand**. It must not be biased toward one methodology universally. Instead:
    ...
    * The agent’s context should prepare it to possibly **ask the user what style they prefer** or make an informed suggestion based on the domain (for example, “Given that this is an air traffic control system, I recommend we create a formal model in Z for precision. Does that sound acceptable?”).

* **Example Domain Models (for Reference):** While not the top priority, the prompt can instruct GPT-5-Pro to collect a couple of **illustrative examples** of domain models from literature, especially those that are not overly specific (so they can be shown as generic patterns). For instance:
...
* All factual statements and definitions must be **supported by citations** from the sources GPT-5 finds, in the format 【source†lines】. This not only gives credibility but also allows the user (or developer) to verify and further read the original references. For example, if stating “A good domain model serves as a knowledge repository and reference for resolving ambiguities” ensure it’s cited from a source . We expect citations especially for definitions (e.g., what is a domain, what is domain engineering), key methodology points (facets, DDD concepts), and any statistics or strong claims.

* If images or figures are found (for instance, a diagram of domain engineering vs application engineering phases), the prompt may instruct GPT-5-Pro to include them using the \!\[alt text\]() or 【source†embed\_image】 format with a proper citation. However, image searching is not a primary goal; only include images if they are immediately useful and found during the research (do not spend effort solely on image hunting). Any embedded image must be cited at the start of the corresponding paragraph as per instructions.

* The **tone** of the compiled context should be **technical, precise, and actionable**. It’s essentially programming the behavior of the domain-modeling agent, so it can include directive language (e.g., “The agent should do X…”) alongside explanatory content. It should read like a knowledge base and instruction manual for performing domain modeling.

* The final compiled prompt should **cover multiple perspectives impartially**. It should not advocate only one methodology; rather, it should educate the agent about each and when it is appropriate. By presenting Bjørner’s formal approach alongside DDD and product-line engineering, the agent will not be biased and can draw on the right approach for the situation at hand.
...
By assembling information and guidelines in this way, GPT-5-Pro will produce a thorough context document. This document will effectively **“train” the coding assistant agent in domain modeling** — giving it the theoretical foundation (from domain science and engineering research) and practical guidance (steps and examples) to collaborate with a user on building machine-readable domain models for complex, critical systems.

When writing the prompt for GPT-5-Pro, include all the above points with appropriate citations, ensuring the content is as **comprehensive** as possible. The end result will be an exhaustive Markdown prompt that defines the domain modeling agent’s knowledge base and behavior, ready to be used in an LLM-driven development session.